...
class HydroSemidistModelGOF(AbsObjectiveFunc):
    ...
    def objective(self, solution):
        metrics = self.exec_function_r(
            self.model_used, solution, self.basin_code, self.prev_q
        )
        if self.metric == "MSE":
            return float(metrics[0])
        elif self.metric == "RMSE":
...
class HydroFullModelGOF(AbsObjectiveFunc):
    def __init__(
        ...
        inf_lim = np.tile(np.array([1e-5, 10, 0, 1e-5, 1e-5, 2, -6]), len(basin_df))
    ...
    def objective(self, solution):
        ...
        for i, idx in enumerate(self.basin_df.index):
            ...
            result_q[basin_code] = self.get_basin_q(
                self.model_used, solution_basin, basin_code, prev_q
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        my_art = AsciiArt.from_url(
            "http://i0.kym-cdn.com/photos/images/facebook/001/244/891/d1f.png"
        )
