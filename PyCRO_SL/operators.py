...
def simplePSO(solution, population, c1, c2):
    fits = [i.get_fitness() for i in population]
    best_idx = fits.index(fits.max())
    best_sol = population[best_idx].solution

    v_prev = np.random.normal((best_sol + solution)/2, best_sol - solution)
    v = v_prev + c1 * random.random() * (best_sol - solution) + c2 * random.random() *  (best_sol + solution)/2
    return solution + v
